...
class vector:
    ...
    def __add__(self, other):  # self + other
        if type(other) in [int, float]:
            ...
            raise TypeError(
                "Argument has to be either scalar or vector, got %r instead" % other
            )
    ...
    def writeVec(self, filename, mode="w"):
        ...
        if ext == ".H":
            ...
            fmt = ">f"
            if (
                self.getNdArray().dtype == np.complex64
                or self.getNdArray().dtype == np.complex128
            ):
                format = ">c8"
            with open(binfile, mode + "b") as f:
                ...
                if np.isfortran(
                    self.getNdArray()
                ):  # Forcing column-wise binary writing
        ...
        # numpy dictionary
            ...
            if mode not in "a":
                if self.ax_info:  # TODO fix saving of ax_info
                    ...
                    for ii, ax_info in enumerate(self.ax_info):
                        axes["%s" % ii + 1] = dict(
                            n=ax_info[0], o=ax_info[1], d=ax_info[2], label=ax_info[3]
                        )

                    np.save(
                        file=filename,
                        arr=dict(arr=self.getNdArray(), ax_info=axes),
                        allow_pickle=True,
                    )
                else:
    ...
    def append(self, vec_in, copy=True):
        ...
        if self.vecSet:
            if not self.vecSet[0].checkSame(vec_in):
                raise ValueError(
                    "ERROR! Provided vector not in the same Space of the vector set"
                )
...
class superVector(vector):
    ...
    def checkSame(self, other):
        ...
        checkspace = np.asarray(
            [self.vecs[idx].checkSame(other.vecs[idx]) for idx in range(self.n)]
        )
    ...
    def isDifferent(self, vecs_in):
        ...
        if type(vecs_in) is not superVector:
            raise TypeError("Input variable is not a superVector")
        return any(
            [self.vecs[idx].isDifferent(vecs_in.vecs[idx]) for idx in range(self.n)]
        )
...
class vectorIC(vector):
    ...
    def reciprocal(self):
        self.getNdArray()[:] = 1.0 / self.getNdArray()
        return self
    ...
    def maximum(self, vec2):
        if np.isscalar(vec2):
            ...
            if not self.checkSame(vec2):
                raise ValueError(
                    "Dimensionality not equal: self = %s; vec2 = %s"
                    % (self.shape, vec2.shape)
                )
    ...
    def dot(self, vec2):
        ...
        if self.size != vec2.size:
            raise ValueError(
                "Vector size mismatching: vec1 = %d; vec2 = %d" % (self.size, vec2.size)
            )
        # Checking dimensionality
        if not self.checkSame(vec2):
            raise ValueError(
    ...
    def multiply(self, vec2):
        ...
        if self.size != vec2.size:
            raise ValueError(
                "Vector size mismatching: vec1 = %s; vec2 = %s" % (self.size, vec2.size)
            )
        # Checking dimensionality
        if not self.checkSame(vec2):
            raise ValueError(
    ...
    def clipVector(self, low, high):
        ...
        self.getNdArray()[:] = np.minimum(
            np.maximum(low.getNdArray(), self.getNdArray()), high.getNdArray()
        )
        return self
...
class vectorOC(vector):
    ...
    def zero(self):
        """Function to zero out a vector"""
        sys_util.RunShellCmd(
            "head -c %s </dev/zero > %s" % (self.size * 4, self.binfile),
            get_stat=False,
            get_output=False,
        )
    ...
    def scale(self, sc):
        ...
        sys_util.RunShellCmd(
            "Solver_ops file1=%s scale1_r=%s op=scale" % (self.vecfile, sc),
            get_stat=False,
            get_output=False,
        )
        return
    ...
    def rand(self, snr=1.0):
        ...
        rms = sys_util.RunShellCmd(
            "Attr < %s want=rms param=1 maxsize=5000" % (self.vecfile), get_stat=False
        )[0]
    ...
    def writeVec(self, filename, mode="w"):
        ...
        if not (os.path.isfile(binfile) and mode in "a"):
            ...
            # Writing file if
            with open(binfile, mode + "b") as fid, open(
                self.binfile, "rb"
            ) as fid_toread:
                while True:
    ...
    def copy(self, vec2):
        ...
        if not self.checkSame(vec2):
            raise ValueError(
                "ERROR! Vector dimensionality mismatching: vec1 = %s; vec2 = %s"
                % (self.shape, vec2.shape)
            )
    ...
    def scaleAdd(self, vec2, sc1=1.0, sc2=1.0):
        ...
        # Performing scaling and addition
        cmd = (
            "Solver_ops file1=%s scale1_r=%s file2=%s scale2_r=%s op=scale_addscale"
